...
(defconstant +uv-error-enum+
  (list
   (cons :UV_E2BIG -7)
   (cons :UV_EACCES -13)
   (cons :UV_EADDRINUSE -98)
   (cons :UV_EADDRNOTAVAIL -99)
   (cons :UV_EAFNOSUPPORT -97)
   (cons :UV_EAGAIN -11)
   (cons :UV_EAI_ADDRFAMILY -3000)
   (cons :UV_EAI_AGAIN -3001)
   (cons :UV_EAI_BADFLAGS -3002)
   (cons :UV_EAI_BADHINTS -3013)
   (cons :UV_EAI_CANCELED -3003)
   (cons :UV_EAI_FAIL -3004)
   (cons :UV_EAI_FAMILY -3005)
   (cons :UV_EAI_MEMORY -3006)
   (cons :UV_EAI_NODATA -3007)
   (cons :UV_EAI_NONAME -3008)
   (cons :UV_EAI_OVERFLOW -3009)
   (cons :UV_EAI_PROTOCOL -3014)
   (cons :UV_EAI_SERVICE -3010)
   (cons :UV_EAI_SOCKTYPE -3011)
   (cons :UV_EALREADY -114)
   (cons :UV_EBADF -9)
   (cons :UV_EBUSY -16)
   (cons :UV_ECANCELED -125)
   (cons :UV_ECHARSET -4080)
   (cons :UV_ECONNABORTED -103)
   (cons :UV_ECONNREFUSED -111)
   (cons :UV_ECONNRESET -104)
   (cons :UV_EDESTADDRREQ -89)
   (cons :UV_EEXIST -17)
   (cons :UV_EFAULT -14)
   (cons :UV_EFBIG -27)
   (cons :UV_EHOSTUNREACH -113)
   (cons :UV_EINTR -4)
   (cons :UV_EINVAL -22)
   (cons :UV_EIO -5)
   (cons :UV_EISCONN -106)
   ...
   (cons :UV_EMFILE -24)
   (cons :UV_EMSGSIZE -90)
   (cons :UV_ENAMETOOLONG -36)
   (cons :UV_ENETDOWN -100)
   (cons :UV_ENETUNREACH -101)
   (cons :UV_ENFILE -23)
   (cons :UV_ENOBUFS -105)
   (cons :UV_ENODEV -19)
   (cons :UV_ENOENT -2)
   (cons :UV_ENOMEM -12)
   (cons :UV_ENONET -64)
   (cons :UV_ENOPROTOOPT -92)
   (cons :UV_ENOSPC -28)
   (cons :UV_ENOSYS -38)
   (cons :UV_ENOTCONN -107)
   (cons :UV_ENOTDIR -20)
   (cons :UV_ENOTEMPTY -39)
   (cons :UV_ENOTSOCK -88)
   (cons :UV_ENOTSUP -95)
   (cons :UV_EOVERFLOW -75)
   (cons :UV_EPERM -1)
   (cons :UV_EPIPE -32)
   (cons :UV_EPROTO -71)
   (cons :UV_EPROTONOSUPPORT -93)
   (cons :UV_EPROTOTYPE -91)
   (cons :UV_ERANGE -34)
   (cons :UV_EROFS -30)
   (cons :UV_ESHUTDOWN -108)
   (cons :UV_ESPIPE -29)
   (cons :UV_ESRCH -3)
   (cons :UV_ETIMEDOUT -110)
   (cons :UV_ETXTBSY -26)
   (cons :UV_EXDEV -18)
   (cons :UV_UNKNOWN -4094)
   (cons :UV_EOF -4095)
   (cons :UV_ENXIO -6)
   (cons :UV_EMLINK -31)
   (cons :UV_EHOSTDOWN -112)
   (cons :UV_EREMOTEIO -121)
   (cons :UV_ENOTTY -25)
   (cons :UV_EFTYPE -4028)
   (cons :UV_EILSEQ -84)
   (cons :UV_ESOCKTNOSUPPORT -94)))
...
(defconstant +uv-handle-enum+
  (list
   (cons :UV_UNKNOWN_HANDLE 0)
   (cons :UV_ASYNC 1)
   (cons :UV_CHECK 2)
   (cons :UV_FS_EVENT 3)
   (cons :UV_FS_POLL 4)
...
(defconstant +uv-req-enum+
  (list
   (cons :UV_UNKNOWN_REQ 0)
   (cons :UV_REQ 1)
   (cons :UV_CONNECT 2)
   (cons :UV_WRITE 3)
   (cons :UV_SHUTDOWN 4)
   ...
   (cons :UV_FS 6)
   (cons :UV_WORK 7)
   (cons :UV_GETADDRINFO 8)
   (cons :UV_GETNAMEINFO 9)
   (cons :UV_RANDOM 10)
   (cons :UV_REQ_TYPE_MAX 11)))
